...
static void Init_TIM16_TX()
{
  ...
  DMAMUX1_Channel9->CCR = (83 << DMAMUX_CxCR_DMAREQ_ID_Pos); // TIM16_UP

  // TX DMA TIM16
  DMA2_Channel4->CPAR = (unsigned long)&TIM16->CCR1;
  DMA2_Channel4->CMAR = (unsigned long)DSHOT_TX_Buff[DSHOT_TIM16];
  DMA2_Channel4->CCR |= DMA_CCR_DIR | DMA_CCR_MINC |  DMA_CCR_MSIZE_0 | DMA_CCR_PSIZE_0;
}
//------------------------------------------------------------------------------
     
static void Init_TIM17_TX()
{
  TIM17->CR1 = 0;
  ...
  DMAMUX1_Channel10->CCR = (85 << DMAMUX_CxCR_DMAREQ_ID_Pos); // TIM16_UP

  // TX DMA TIM17
  DMA2_Channel5->CPAR = (unsigned long)&TIM17->CCR1;
  DMA2_Channel5->CMAR = (unsigned long)DSHOT_TX_Buff[DSHOT_TIM17];
  DMA2_Channel5->CCR |= DMA_CCR_DIR | DMA_CCR_MINC |  DMA_CCR_MSIZE_0 | DMA_CCR_PSIZE_0;
}
//------------------------------------------------------------------------------
...
bool DSHOT_Init(unsigned long Freq, bool Bidirect)
{
  ...
  if(RCC->APB2ENR & RCC_APB2ENR_TIM16EN)   return false;
  if(RCC->APB2ENR & RCC_APB2ENR_TIM17EN)   return false;
...
bool DSHOT_SetCommand(unsigned short Command[6], bool Forced)
{
  ...
  ds_packet1(Command[4] & 0x0FFF, Command[4] >> 15, DSHOT_TX_Buff[DSHOT_TIM16]);
  ds_packet1(Command[5] & 0x0FFF, Command[5] >> 15, DSHOT_TX_Buff[DSHOT_TIM17]);
  ...
  TIM16->DIER = TIM17->DIER = TIM_DIER_UDE;
