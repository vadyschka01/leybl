...
struct EnginePowerInfo
{
  ...
  unsigned short engineSpeed;//Текущие оборты в секунду мотора
  short current; // Ток потребляемый мотором
  short voltage; // Напряжение на моторе
  short temp; // Температура мотора
...
typedef void (*SendCallback)(void* data, unsigned long size, int priority);
typedef void (*SendTelemCallback)(void* data, unsigned long size);
...
void RawCalibData();
extern unsigned long LastTimeRecvMsg;
