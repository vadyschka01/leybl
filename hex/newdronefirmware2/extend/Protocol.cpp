...
static void ProcessingCommandSetCalibrationData(MessageCommandSetCalibrationData* msg)
{
  ...
  switch(msg->sensor_id)
  {
    ...
    {      
      LevelHor.Pitch = DroneOrintation.PRY.X;
      LevelHor.Roll = DroneOrintation.PRY.Y;
      LevelHor.Yaw = DroneOrintation.PRY.Z;
      MainDrone.NeedSaveNewFLASHData = true;
      return;
    }
  }
...
static void DeviceStatusesInfo()
{
  MessageDeviceStatuses Msg;
  Msg.headers.CreateMessageHeaders(sizeof(MessageDeviceStatuses), MESSAGES_ID::DeviceStatusesInfo);
  Msg.ImuCalib = !CalibDataIMU.CalibAccNeed;
  Msg.MagCalib = !ExternMagCalibData.CalibNeed;
  Msg.GpsInit = MainDrone.GPSInit;
  Msg.LidarInit = MainDrone.TOFInit;
  Msg.OpticalFlowInit = MainDrone.OFInit;
  Msg.CamFlowInit = MainDrone.CamFlowInit;
  SendMes(Msg.headers.header, 2);
}
...
static void ProcessingMessage()
{
  ...
  case MESSAGES_ID::DeviceStatusesInfo: DeviceStatusesInfo();return;
  }
}