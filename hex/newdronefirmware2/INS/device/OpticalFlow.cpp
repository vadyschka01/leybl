...
void OpticalFlow::SetMove(const Vector2* Move, float Range, float Quality)
{
  ...
  //a_spd += s_spd.Power(RegSpd.Power) * RegSpd.Alpha * alpha; // Сомнительно для скорости
  ...
  BufferPos += spd * trust * Period; // Восстановление позиции со сглаживанием (без потери точности)
  
  //Vector2 sub = BufferPos * BufferAlpha;
  //BufferPos-=sub;
  
  Shift.Pos = spd * trust * Period * BufferAlpha;
