...
void Engine::GetHex(float Thrust[6])
{
  float throt = sqrtf(ThrustThrot);
  ...
  Thrust[0] = Thrust[1] = Thrust[2] = Thrust[3] = Thrust[4] = Thrust[5] = 0;
  
  /*Thrust[HexScheme[1]] = - roll + pitch + yaw; // Передний Правый (CCW)
  Thrust[HexScheme[4]] = + roll - pitch + yaw; // Задний Левый (CW)
  Thrust[HexScheme[0]] = + roll + pitch - yaw; // Передний Левый (CW)
  Thrust[HexScheme[5]] = - roll - pitch - yaw; // Задний Правый (CCW)
  Thrust[HexScheme[3]] = - roll + 0.00f - yaw; // Боковой Правый (CW)
  Thrust[HexScheme[2]] = + roll + 0.00f + yaw; // Боковой Левый (CCW)*/
  
  Thrust[2] = +pitch - yaw; // U  (CW)
  Thrust[0] = -pitch + yaw; // D  (CCW)
  
  Thrust[4] = -roll + pitch*0.5f + yaw; // RU (CCW)
  Thrust[3] = -roll - pitch*0.5f - yaw; // RD (CW)
  
  Thrust[1] = +roll + pitch*0.5f + yaw; // LU (CCW)
  Thrust[5] = +roll - pitch*0.5f - yaw; // LD (CW)
