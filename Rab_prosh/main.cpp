...
static bool CAMFLOW_ONLINE = false;
...
MOTOR_TYPE MotorType = MOTOR_TYPE::PWM;
...
void SetMotorPower(unsigned long Count, unsigned short Pow[6], unsigned short Min, unsigned short Max)
{
  ...
  switch(MotorType)
  {
    ...
    {
      unsigned short cmd[6]={Pow[2], Pow[1], Pow[3], Pow[0], 0, 0}; 
      if(Count==6) { cmd[4]=Pow[4];  cmd[5]=Pow[5]; }
...
void AutoControl(Vector3 gyro)
{
  ...
  if (MainDrone.engine == ENGINE_STATUS::disable)
  {
    ...
    {
      unsigned short Power[4];
      Power[0] = 0;
      Power[1] = 0;
      Power[2] = 0;
      Power[3] = 0;
      Power[MainDrone.MotorTestId] = 100;
      SetMotorPower(4, Power, 0, 100);
  ...
  else
  {
    ...
    if(DroneGoal.New)
    {
      ...
      MainVel.SetDistance(PointBegin, DroneGoal.Position, 1.0f); // Время нужно считать кому то в зависимости от массы и маршрута !!!!!!!!!!!!!!!
  ...
  unsigned short power[4] = { (short)(motor[0]*1000.0f), (short)(motor[1]*1000.0f), (short)(motor[2]*1000.0f), (short)(motor[3]*1000.0f) };
  
  //power[0]=power[1]=power[2]=power[3]=Joypad.Z/2;
  
  static unsigned short testPower[4];
  testPower[0] = power[0];
  testPower[1] = power[1];
  testPower[2] = power[2];
  testPower[3] = power[3];
  
  SetMotorPower(4, power, 100, 1000);
}
...
void ReadStateINS()
{
  ...
  { //Battery
    ...
    bat.attackStatus = AnalyserModule.lastStatus;
    bat.attackTime = AnalyserModule.TimeDetect;
    //-------------------------------------------------------------------------------
  ...
  {// INNER
    ...
    //inner.yawVel = (DroneInertial.Pos.X - DataGPS.X) * 10.0f;//MainIRS.Inertial.Spd.Z;//DataGPS.Z;// / MainOF.RegShift.Accuracy;//MainIRS.Inertial.Spd.Z;//DroneInertial.Spd.Y;
    //inner.rollVel = (DroneInertial.Pos.Y - DataGPS.Y) * 10.0f;//MainOF.RegShift.Test;//DroneInertial.Spd.Y;//MainVel.PointBegin.Y;//PointLand.Y; //DataGPS.X;//MainGPS.Shift.Dyn.Y / MainGPS.RegShift.Accuracy;//
    
    inner.yawVel = MainOF.ShiftSpd.X * 10.0f;
    inner.rollVel = MainOF.ShiftSpd.Y * 10.0f;
...
void InitPDI()
{
  ...
  MainAtt.RegAngleP = { IRS::Period, 20.0f,  10.0f,  0.0f,  { 0.0f, 0.0f }, true };
  MainAtt.RegAngleR = { IRS::Period, 20.0f,  10.0f,  0.0f,  { 0.0f, 0.0f }, true };
  MainAtt.RegAngleY = { IRS::Period, 20.0f,  10.0f,  0.0f,  { 0.0f, 0.0f }, true };
                                                                                    
  MainAtt.RegSpeedP = { IRS::Period, 0.50f,  0.04f,  0.0f,  { 0.0f, 0.0f }, true };
  MainAtt.RegSpeedR = { IRS::Period, 0.50f,  0.04f,  0.0f,  { 0.0f, 0.0f }, true };
  MainAtt.RegSpeedY = { IRS::Period, 1.00f,  0.20f,  0.0f,  { 0.0f, 0.0f }, true };
                                            
  //                             (лимит наклонов)
  MainVel.RegSpeedX = { IRS::Period, 0.383f,  0.1f,   0.0f};
  MainVel.RegSpeedY = { IRS::Period, 0.383f,  0.1f,   0.0f};
...
void MEM_Save() // Записать всё во флеш 
{
  ...
  for(int a=0; a<count; a++)
  {
    ...
    if (FirstBytes) //Харним во флеше CRC дерева параметров
    {
      ...
      for(int a=0; a<sizeof(Settings::MenuParametersCount); a++) crc+=Settings::MenuParametersCount[a];
      memcpy(buffer, &crc, 2);
      len = 2;
      FirstBytes = false;
    }
...
void MEM_Restore() // Восстановить всё из флеша
{
  ...
  if(FlashVersion != *(unsigned long*)(mem+2)) MEM_Save(); //Это ненормальный флэш
...
int main()
{
  ...
  /*while(1)
  {
    static unsigned short cmd[6]={0,0,0,0,0,0};
    DSHOT_SetCommand(cmd, true);
    for(volatile int a=0; a<100000; a++);
    static unsigned long qqq=0;
    qqq++;
  }*/
  ...
  while(true)
  {
    ...
    Point gps_p;
    static Point LastGpsPoint = {0.0,0.0, -1234567.0f};
    ...
    if(GPS_Update())
    {
      ...
      {
        if (MainDrone.GPSInit) AnalyserModule.update = true;
        //valid=true;
        if(valid)
        {
          if (LastGpsPoint.Altitude == -1234567.0f) LastGpsPoint = gps_p;
    ...
    //AnalyserModule.update = true;
    if (AnalyserModule.update && LastGpsPoint.Altitude != -1234567.0f)
    { 
      static float dx=0, dy=0, dz=0;
      GPS_LocalDistance(LastGpsPoint, gps_p, dx, dy, dz);
      float distanse = sqrtf(dx*dx + dy*dy);
      static GPS_BaseInfo gpsInfoAnalyser;
      GPS_GetBaseInfo(gpsInfoAnalyser);
      
      Data_Analyser data;
      data.values[PARAMETERS::POSITION_DIST] = distanse;
      data.values[PARAMETERS::ABS_ALT] = gpsInfoAnalyser.absAlt;
      data.values[PARAMETERS::REAL_ALT] = gpsInfoAnalyser.realAlt;
      data.values[PARAMETERS::HDOP] = gpsInfoAnalyser.hdop;
      data.values[PARAMETERS::VDOP] = gpsInfoAnalyser.vdop;
      data.values[PARAMETERS::PDOP] = gpsInfoAnalyser.pdop;
      data.values[PARAMETERS::NOISE] = gpsInfoAnalyser.noise;
      data.values[PARAMETERS::SAT_VISIBLE] = gpsInfoAnalyser.satVisible;
      data.values[PARAMETERS::SAT_USED] = gpsInfoAnalyser.satUsed;

      AnalyserModule.process(&data, TICK_GetCount());
    ...
    if(CamFlow_Update())
    {
      ...
      if (CamFlow_GetData(xyy))
      {
        ...
        if (!CAMFLOW_ONLINE) CAMFLOW_ONLINE = true;
        
        //CamFlow_Send(MainIRS.Inertial.Pos.Z - MainIRS.GroundShift);
      }
    }
    if (CAMFLOW_ONLINE && TICK_GetCount() - DataCamFlow.Time > 1000) CAMFLOW_ONLINE = false; // Сбрасываем, если за 1 сек данных не было
