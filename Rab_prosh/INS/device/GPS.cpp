...
void GPS::SetPosition(const Vector3* Position, float Time)
{
  ...
  a_pos += a_pos.Power(RegPos.Power) * Vector3(RegPos.Alpha, RegPos.Alpha, AlphaHeight) * alpha;
  a_spd += s_spd.Power(RegSpd.Power) * Vector3(RegSpd.Alpha, RegSpd.Alpha, AlphaHeight) * alpha;
