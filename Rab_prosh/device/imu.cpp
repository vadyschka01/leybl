...
static const unsigned char IMU_Addr = 0x68; // ICM-20948 AccGyr
static const unsigned char MAG_Addr = 0x0C; // ICM-20948 Mag - AK09916
...
IMU_Proc IMU_DoneProc;

IMU_Calib_Data CalibDataIMU;

struct IMU_DataBufer { short ax; short ay; short az; short gx; short gy; short gz; short temp; short mx; short my; short mz; };

static bool Calibrated = false;

static long GyroShift[3]{0, 0, 0};

void IMU_Calibrate(IMU_DataBufer& imu, long GyrLim, long AccZero, long AccMax)
{
...
/*const int AccMedCount = 7;
short AccMedBuf[3][AccMedCount], AccMedTmp[3][AccMedCount];
MED_Data16 AccMed[3] = { { AccMedCount, 0, AccMedBuf[0], AccMedTmp[0]}, { AccMedCount, 0, AccMedBuf[1], AccMedTmp[1]}, { AccMedCount, 0, AccMedBuf[2], AccMedTmp[2]} };

short GyrMedBuf[3][7], GyrMedTmp[3][7];
MED_Data16 GyrMed[3] = { { 7, 0, GyrMedBuf[0], GyrMedTmp[0]}, { 7, 0, GyrMedBuf[1], GyrMedTmp[1]}, { 7, 0, GyrMedBuf[2], GyrMedTmp[2]} };

#define WINDOW_SIZE 50

// Структура для фильтра скользящего среднего для одной оси
typedef struct {
...
BiquadFilter FiltAcc[3];
BiquadFilter FiltGyr[3];
...
static void IMU_CallbackProc(unsigned char Address, const unsigned char* Data, unsigned char Size)
{
  ...
  //data.ax=MED_Update(data.ax, AccMed[0]);
  //data.ay=MED_Update(data.ay, AccMed[1]);
  //data.az=MED_Update(data.az, AccMed[2]);
  
  //data.gx=MED_Update(data.gx, GyrMed[0]);
  //data.gy=MED_Update(data.gy, GyrMed[1]);
  //data.gz=MED_Update(data.gz, GyrMed[2]);
  
  //data.ax=update_filter(&filter_x, data.ax);
  //data.ay=update_filter(&filter_y, data.ay);
  //data.az=update_filter(&filter_z, data.az);
...
static inline void IMU_SetBank(unsigned char Bank)
{
  unsigned char reg[2];
...
static inline unsigned char MAG_GetReg(unsigned char Slave, unsigned char Reg, unsigned char Len=0)
{
  IMU_SetBank(3);
  ...
  if(!Len) 
  {
    while(IMU_GetReg(0x17)!=0x40); // I2C_MST_STATUS  -  I2C_SLV4_DONE
    IMU_SetBank(3);
    return IMU_GetReg(0x17); // I2C_SLV4_DI
  }
  return 0;
}
//------------------------------------------------------------------------------

static inline void MAG_SetReg(unsigned char Slave, unsigned char Reg, unsigned char Value)
{
  IMU_SetBank(3);
...
{ 
  ...
  //if(!I2C1_CheckDevice(IMU_Addr)) return false;
  
  for(int a=0; a<100000; a++) { asm volatile("NOP"); } // Подождем готовности устройства к работе

  IMU_SetReg(0x06, 0x80); // Сброс устройства
  for(int a=0; a<100000; a++) { asm volatile("NOP"); }
  
  IMU_SetReg(0x06, 0x01); // Включить
  for(int a=0; a<100000; a++) { asm volatile("NOP"); }
  
  IMU_SetBank(2);
  IMU_SetReg(0x01, GYRO_DLPFCFG_73 | GYRO_FS_SEL_2000 | GYRO_FCHOICE_ON); // GYRO_CONFIG_1
  IMU_SetReg(0x14, ACCEL_DLPFCFG_69 | ACCEL_FS_SEL_8 | ACCEL_FCHOICE_ON); // ACCEL_CONFIG
  
  // MAG
  IMU_SetBank(0); IMU_SetReg(0x15, 0x00); // INT_PIN_CFG   -   OFF BYPASS_EN
  
  IMU_SetBank(3); IMU_SetReg(0x01, 0x17); // I2C_MST_CTRL   -   I2C_MST_P_NSR  | (345.60Hz 46.67%)=15
  
  IMU_SetBank(0); IMU_SetReg(0x03, 0x20); // USER_CTRL   -   I2C_MST_EN
...
void IMU_GetAsunc(const IMU_Proc& DoneProc)
{
  ...
  unsigned char reg=0x2D;
  I2C1_Trans(&IMU_Device, IMU_Addr, &reg, 1, sizeof(IMU_DataBufer));
}
//------------------------------------------------------------------------------
