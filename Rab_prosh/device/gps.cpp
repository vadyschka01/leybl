...
static GPS_BaseInfo GPS_BaseInfoBuffers[3];
static unsigned int idx_write = 0; // Write
static unsigned int idx_ready = 1; // Update data
static unsigned int idx_read  = 2; // Read data

static void GPS_CallBack_GSV(GPS_PROTO Proto, void* Data)
{
   auto commit_and_forward = []() {
      int temp = 0;
      do // Try lock
      {
        temp = __LDREXW(&idx_ready); // Read lock+id
        
      } while (__STREXW(idx_write, &idx_ready)); 
      idx_write = temp;
      
      GPS_BaseInfoBuffers[idx_write] = GPS_BaseInfoBuffers[idx_ready];
  };
  
  
  GPS_BaseInfo& current_write_buf = GPS_BaseInfoBuffers[idx_write];
  ...
  if(Proto==GPS_PROTO::GNGGA)
  {
    ...
    current_write_buf.absAlt = Base_Geo;
    
    Base_Alt=Base_Geo-Base_BeginXYZ.Altitude;
    current_write_buf.realAlt = Base_Alt;
    //---
    ...
    if(data->Satellites[0] && GetCoordinates_Valid) current_write_buf.satUsed = strtoul(data->Satellites, 0, 10);
    
    if(data->Quality[0] && GetCoordinates_Valid) current_write_buf.fixType = strtoul(data->Quality, 0, 10);
  ...
  else if(Proto==GPS_PROTO::GLGSV)
  {
    ...
    if(*data->Count==*data->Index)
    {
      ...
      if(count) current_write_buf.noise=noise/count;
      else current_write_buf.noise=0;
      
      current_write_buf.satVisible=GPS_VisibleGPS+GPS_VisibleGLO;
  ...
  else if(Proto==GPS_PROTO::GNRMC)
  {
    ...
    current_write_buf.lat=GetCoordinates_Coord.Latitude;
    current_write_buf.lon=GetCoordinates_Coord.Longitude;
    
    if(data->HorSpeed[0]) current_write_buf.speed=strtof(data->HorSpeed, 0);
    
    if(data->Time[0]) current_write_buf.timeUTC=strtof(data->Time, 0);
    
    float dx, dy, dz;
    GPS_LocalDistance(Base_BeginXYZ, {current_write_buf.lat, current_write_buf.lon}, dx, dy, dz);
                      
    current_write_buf.LocalXYZ[0] = dx;
    current_write_buf.LocalXYZ[1] = -dy;
    current_write_buf.LocalXYZ[2] = Base_Geo-Base_BeginXYZ.Altitude;
  ...
  else if(Proto==GPS_PROTO::GNGSA)
  {
    ...
    if(data->HDOP[0]) current_write_buf.hdop=strtof(data->HDOP, 0);
    if(data->VDOP[0]) current_write_buf.vdop=strtof(data->VDOP, 0);
    if(data->PDOP[0]) current_write_buf.pdop=strtof(data->PDOP, 0);
...
{
  //static Point TESTGPS = {47.32419968, 38.77563477, 44};
  //static bool TESTGPSVALID = false;
  //Coord = TESTGPS;
  //Valid = TESTGPSVALID;
  //return TESTGPSVALID;
...
bool GPS_GetBaseInfo(GPS_BaseInfo& Info)
{
  ...
  } while (__STREXW(idx_read, &idx_ready)); 
  idx_read = temp;
  
  Info = GPS_BaseInfoBuffers[idx_read];
